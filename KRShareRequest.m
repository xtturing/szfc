...
@interface KRShare (KRShareRequest)
- (void)requestDidFinish:(KRShareRequest *)request;
- (void)requestDidFailWithInvalidToken:(NSError *)error;
@end

@interface KRShareRequest (Private)
...
- (void)handleResponseData:(NSData *)data 
{
    ...
    if(result == nil && [tmpStr hasPrefix:@"access_token="] && [(KRShare *)delegate shareTarget] == KRShareTargetTencentblog)
    {
	...
	else
	{
        ...
        if (error_code != 0)
        {
            ...
            {
                [krShare requestDidFailWithInvalidToken:error];
            }
            else
            {
                [self failedWithError:error];
...
{
    KRShareRequest *request = [[[KRShareRequest alloc] init] autorelease];
                                    ...
                                    delegate:(id<KRShareRequestDelegate>)delegate
{
    ...
    return [KRShareRequest requestWithURL:url
...
- (void)connect
{
    ...
    [NSMutableURLRequest requestWithURL:[NSURL URLWithString:urlString]
                        ...
                        timeoutInterval:kKRShareRequestTimeOutInterval];NSLog(@"%@",urlString);
