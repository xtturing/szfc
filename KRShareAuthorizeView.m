...
@implementation KRShareAuthorizeView
...
- (id)initWithAuthParams:(NSDictionary *)params
                delegate:(id<KRShareAuthorizeViewDelegate>)_delegate
{
    if ((self = [self init]))
    {
...
- (void)sizeToFitOrientation:(BOOL)transform
{
    ...
    CGFloat scaleFactor = KRShareIsDeviceIPad() ? 0.6f : 1.0f;
...
- (void)load
{
    ...
    KRShare *krShare = (KRShare *)delegate;
...
- (BOOL)webView:(UIWebView *)aWebView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    {
        NSString *error_code = [KRShareRequest getParamValueFromUrl:url paramName:@"error_code"];
        
        if (error_code)
        {
            NSString *error = [KRShareRequest getParamValueFromUrl:url paramName:@"error"];
            NSString *error_uri = [KRShareRequest getParamValueFromUrl:url paramName:@"error_uri"];
            NSString *error_description = [KRShareRequest getParamValueFromUrl:url paramName:@"error_description"];
        ...
        {
            NSString *code = [KRShareRequest getParamValueFromUrl:url paramName:@"code"];
            if (code)
            {
                [self hide];
