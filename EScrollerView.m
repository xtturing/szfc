	...
	if ((self=[super initWithFrame:rect])) {
        ...
        for (int i=0; i<pageCount; i++) {
            ...
            if ([imgURL hasPrefix:@"http://"]) {
                //网络图片 请使用ego异步图片库
//                imageView = [[EGOImageView alloc] initWithPlaceholderImage:[UIImage imageNamed:@"icon.png"]];
                UIImageView *imageView=[[[UIImageView alloc] init] autorelease];
                [imageView setImageWithURL:[NSURL URLWithString:imgURL]];
                [imageView setFrame:CGRectMake(viewSize.size.width*i, 0,viewSize.size.width, viewSize.size.height)];
...
//        [noteTitle setTextColor:[UIColor colorWithRed:0.197 green:0.592 blue:0.219 alpha:1.000]];
