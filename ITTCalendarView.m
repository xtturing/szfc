...
- (void)setSelectedAtRow:(NSUInteger)row column:(NSUInteger)column selected:(BOOL)selected
{
    ...
    [_selectedGridViewIndicesDic setObject:[NSString stringWithFormat:@"%d", selected] forKey:key];
}

- (BOOL)isSelectedAtRow:(NSUInteger)row column:(NSUInteger)column
{
    NSString *key = [NSString stringWithFormat:@"%d%d", row, column];
    NSString *selectedString = [_selectedGridViewIndicesDic objectForKey:key];
    BOOL selected = FALSE;
    if (selectedString) {
...
{
    ITTDINFO(@"calendarSrollViewTouchesEnded");
    GridIndex index = [self getGridViewIndex:calendarScrollView touches:touches];
    if ([self isValidGridViewIndex:index]) {
        BOOL selectedEnable = TRUE;        
        if (!_moved) {
